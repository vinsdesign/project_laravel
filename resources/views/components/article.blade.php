...
<div
    class="mx-auto mt-5 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-8 border-t border-gray-200 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
        ...
        <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            <div class="flex justify-between items-center mb-5 text-gray-500">
                <a href="/category/{{ $article->Category->slug }}"
                    class="bg-{{$article->Category->color}}-100 text-primary-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-primary-800">
                ...
                <time datetime="{{ $article['date'] }}" class="text-gray-500 text-sm">{{ \Carbon\Carbon::parse($article['date'])->format('M d, Y') }}</time>
                
            </div>
            <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><a href="/blog/{{ $article['slug'] }}">                        
                {{ $article['title'] }}
            </a></h2>
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    ...
                    <a href="/author/{{ $article->author->username }}" class="font-medium dark:text-white">
                        {{ Str::limit($article->author->name, 15) }}
                    </a>
                </div>
